...
void ImageData::render(glm::mat4 mvpView) {
//    m_shader.use();
//    GL_CHECK(glBindVertexArray(m_vaoID));
...
bool ImageData::loadFromMemory(const void *data, std::size_t size) {
    if (data && size) {
        m_pixels.clear();
        int width, height, channels;
...
glm::vec2 ImageData::getSize(void) const {
    return m_size;
}

const Uint8* ImageData::getPixels() const {
    if (!m_pixels.empty()) {
        return &m_pixels[0];
    } else {
